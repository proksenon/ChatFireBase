...
typealias FireCodable = BaseCodable & Codable
...
class FirestoreService {
  ...
  func update<T>(_ object: T, reference: Reference, completion: @escaping CompletionObject<ResponseStatus>) where T: FireCodable {
  ...
  func delete<T>(_ objects: T.Type, reference: Reference, parameter: DataQuery, completion: @escaping CompletionObject<ResponseStatus>) where T: FireCodable {
	let queryReference: Query
	switch parameter.mode {
	case .equal:
	  queryReference = reference.reference().whereField(parameter.key, isEqualTo: parameter.value)
